		...
		<template v-for="(item,index) in listData">
			<li v-if="item.child.length <= 0" :class="{'primary-menu':item.type==='primary'}">
				<a href="javascript:void(0)">
					...
					<input v-if="item.edit" type="text" :value="item.title">
					<!-- <el-input placeholder="请输入内容" v-model="item.title" clearable></el-input> -->
					<button v-if="item.edit" @click="confirmUpdate($event,index)">确认</button> 
					<button v-if="item.edit" @click="update($event,index)">取消</button></a>
				<div class="btn-group">
			...
			<li v-else :class="{'primary-menu':item.type==='primary'}">
				<a href="javascript:void(0)" class="color-blue" @click="showChildMenu(index)">
					...
					<!-- <el-input placeholder="请输入内容" v-model="item.title" clearable></el-input> -->
					<button v-if="item.edit" @click="confirmUpdate($event,index)">确认</button> 
					<button v-if="item.edit" @click="update($event,index)">取消</button>
				</a>
				<div class="btn-group">
					<button v-if="!item.edit" @click="update($event,index)">修改</button>
					<button @click="deleteFn(index)">删除</button>
...
<script>
	import Tree from '@/views/aaa/tree.vue'
