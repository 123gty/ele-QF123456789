		...
		<transition name="form-fade" mode="in-out">
			<section class="form_contianer">
				<div class='titleArea rflex'>
					...
					<span class='title'>郭某<i>自用调试</i></span>
				</div>
				<el-form :model="loginForm" :rules="rules" ref="loginForm" class="loginForm">
					<el-form-item prop="username" class="login-item">
						<span class="fa-tips"><i class="fa fa-user"></i></span>
						<el-input @keyup.enter.native="submitForm('loginForm')" class="area" type="text" placeholder="用户名" v-model="loginForm.username"
						 disabled></el-input>
					</el-form-item>
			...
			submitForm(loginForm) {
				this.$refs[loginForm].validate((valid) => {
					if (valid) {
						...
						console.log(mima)
						// console.log(this.getDAes(mima))解密输出
						setToken(mima)
						this.$router.push({
							name: 'yi'
						});
